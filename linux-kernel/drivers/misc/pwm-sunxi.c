...
{ 
	int init_enable, init_duty_percent, init_period, act_state;
	...
	if (err == 0 && init_enable) {
		...
		err = script_parser_fetch("pwm0_para", "act_state", &act_state,sizeof(act_state)/sizeof(int));
		if (err || (act_state > 1) ||(act_state <0)) {
			pr_err("%s script_parser_fetch '[pwm0_para]' 'act_state' err - default is act hi\n",__func__);
			act_state=1;
		}
	...
	if (err == 0 && init_enable) {
		...
		err = script_parser_fetch("pwm1_para", "act_state", &act_state,sizeof(act_state)/sizeof(int));
		if (err || (act_state > 1) ||(act_state <0)) {
			pr_err("%s script_parser_fetch '[pwm1_para]' 'act_state' err - default is act hi\n",__func__);
			act_state=1;
		}
...
ssize_t pwm_set_mode(unsigned int enable, struct sun4i_pwm_available_channel *chan) { 
	...
	if(enable == 1) { 
		...
		case 0: 
			...
			chan->ctrl_current.s.ch0_clk_gating = 0; 		//TBD!!
			break; 
		case 1: 
			...
			chan->ctrl_current.s.ch1_clk_gating = 0; 		//TBD!!
			break; 
		default: 
			status = -EINVAL; 
			break; 
...
{ 
	unsigned long duty = duty_ns / 1000; 
	unsigned long period = period_ns / 1000; 
	...
	//pwm->chan->duty_percent = -1; /* disable duty_percent if duty is set by hand */ 
	pwm->chan->duty_percent = (duty * 100) / period; 
	//fixup_duty(pwm->chan); 
